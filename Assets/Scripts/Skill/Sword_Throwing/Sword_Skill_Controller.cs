    ...
    private void SpinLogic()
    {
        if (isSpinning)
        {
            ...
            if (wasStopped)
            {
                ...
                transform.position = Vector2.MoveTowards(transform.position, new Vector2(transform.position.x + spinXDirection, transform.position.y + spinYDirection), 1.5f * Time.deltaTime);
                if (spinTimer < 0)
                {
                    isReturn = true;
                    isSpinning = false;
